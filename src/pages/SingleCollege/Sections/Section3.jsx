    ...
    return ( 
        <>
            <div className="college-infotabs mt-5">
                ...
                <div className="all-tabs bg-blue-700 rounded-sm px-4 py-2 md:px-8 md:py-5">

                    <div className="tabs flex">

                        {
                            tabs.map((tab,index)=> (
                                <div className={`text-white text-base cursor-pointer md:text-lg ${activeTab===index && 'border-b-4 border-b-white/25 pb-1 font-semibold bg-white/10' }  px-4 py-2 select-none`} onClick={()=> setActiveTab(index)} >{tab}</div>
                            ))
                        }
                    ...
                    <div className="tab-content">
                        ...
                        {tabContent.map((content,index)=> (
                            <div className={`tabcontent text-white/95 tracking-wide pt-4 pb-14   ${activeTab===index ? 'block' : 'hidden'}`}>
                              {content}
                            </div>
                        ))}
